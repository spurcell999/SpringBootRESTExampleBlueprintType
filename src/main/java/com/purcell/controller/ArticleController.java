...
import com.purcell.ArticleInfo;
import com.purcell.Detail;
import com.purcell.Error;
import com.purcell.Errors;
import com.purcell.entity.Article;
import com.purcell.service.IArticleService;
...
public class ArticleController {
    ...
    public ResponseEntity<Errors> getArticleById(
            @RequestHeader(value = "Accept", defaultValue = MediaType.APPLICATION_JSON_VALUE) String accept,
            @RequestHeader(value = "Content-Type", required = true) String contentType
            ) {
        Errors ei = new Errors();
        ArrayList errorList = new ArrayList<Error>();
        for (int i=0; i<5; i++) {
            Error e = new Error();
            e.setReasonCode("RC");
            e.setRequestId("1234");
            e.setSource("source");
            e.setRecoverable(Boolean.FALSE);
            e.setDescription("descritption");
            Detail dtl = new Detail();
            dtl.setName("scott");
            dtl.setValue("purcell");
            ArrayList array = new ArrayList<Detail>();
            array.add(dtl);
            e.setDetails(array);
            errorList.add(e);
        }

        ei.setErrors(errorList);


        return new ResponseEntity<Errors>(ei, HttpStatus.OK);
    }
